...
#'
#' splitting_criterion_honest_log <- function(split.var,
...
#'   X <- cbind(as.matrix(model_frame[,-1]))
#'
#'   lm_1 <- .lm.fit(X[split_var_values  < split_at, , drop = FALSE], response[split_var_values < split_at])
#'   lm_2 <- .lm.fit(X[split_var_values  >= split_at, , drop = FALSE], response[split_var_values >= split_at])
#'
#'   return(list(sse = min(loss, na.rm = TRUE), split = split_at, lm_1 = lm_1$coefficients, lm_2 = lm_2$coefficients,
#'               var_1 = var(lm_1$residuals), var_2 = var(lm_2$residuals)))
#' }
#'
#' #' @export
#' reg_tree_honest_log <- function(data, formula, split.vars, minsize, mtry = 1/3, data.predict, cl = NULL, log_first = FALSE) {
#'
...
#'   tree_info <- data.frame(NODE = 1, NOBS = nrow(data), FILTER = NA, TERMINAL = "SPLIT",
#'                           beta1 = NA, beta2 = NA, beta3 = NA, var_res = NA,
#'                           stringsAsFactors = FALSE)
#'
...
#'         splitting_criterion_honest_log(kk,
...
#'
#'
#'       children_coefficients <- rbind(c(splitting[[tmp_splitter]]$lm_1, splitting[[tmp_splitter]]$var_1),
#'                                      c(splitting[[tmp_splitter]]$lm_2, splitting[[tmp_splitter]]$var_2))
#'       # colnames(children_coefficients) <- c("beta0", "beta1", "beta2", "beta3")
#'       colnames(children_coefficients) <- c("beta1", "beta2", "beta3", "var_res")
#'
...
#'         prediction_leaf <- unlist(leafs[i, c("beta1", "beta2", "beta3")]) %*% t(cbind(as.matrix(data.predict[ind_prediction, c("rv_lag_1", "rv_lag_5", "rv_lag_22")]))) + 1/2 * leafs[i, c("var_res")]
#'         predictions[ind_prediction] <- exp(t(prediction_leaf))
#'       }
#'     }
#'   }
#'
...
#'       data.predict %>%
#'       select(date, permno, mean_rv) %>%
#'       mutate(forecast = predictions * exp(mean_rv))
#'   }
#'
#'
#'
